...
llvm::Value* CodeGenerator::generateArrayIndexPointer(const ASTNode &binaryOpNode) {
    ...
    // или это что-то ещё)
    llvm::errs() << "[DEBUG] baseVal->getType() = ";
    baseVal->getType()->print(llvm::errs());
    llvm::errs() << "\n";
...
//                llvm::Type *gvarTy = globalVar->getValueType(); // тип, который хранит глобал
//                if (gvarTy->isArrayTy()) {
//                    // (A) Глобальный массив [N x i32] => возвращаем сам globalVar
//                    //     (указатель на [N x i32]), без Load.
//                    return globalVar;
//                } else {
//                    // (B) Глобальная «обычная» переменная (i32, i1, double, ...)
//                    //     => делаем Load, чтобы получить её значение.
...
//                if (allTy->isArrayTy()) {
//                    // (C) Локальный массив => возвращаем саму alloca (указатель на [N x i32]).
//                    return allocInst;
//                } else {
//                    // (D) Локальная скалярная переменная => Load значения.
//                    return builder.CreateLoad(allTy, allocInst, varName + "_val");
//                }
//            }
//
//            throw std::runtime_error(
//                    "Expected global/local variable for identifier: " + varName
//            );
//        }
//
//            // ----------------------------------------------------------------------------
...
//            // ---------------- 3.1) Присваивание ( = ) ----------------
//            if (op == "=") {
//                // node.children[0] — куда присваиваем (Identifier или '[]')
//                // node.children[1] — что присваиваем
//                const ASTNode &lhsNode = node.children[0];
//                llvm::Value *lhsPtr = nullptr; // это pointer, куда будем store
//
//                // (A) Присваиваем простому идентификатору?
//                if (lhsNode.type == ASTNodeType::Identifier) {
//                    lhsPtr = namedValues[lhsNode.value];
//                    if (!lhsPtr) {
//                        // не локальная, пробуем глобальную
...
//                    // Чтобы присвоить array[index] = rhs,
//                    // получаем pointer на элемент (GEP) через generateArrayIndexPointer
//                    llvm::Value *elemPtr = generateArrayIndexPointer(lhsNode);
//                    lhsPtr = elemPtr; // потом store rhsVal в elemPtr
//                } else {
//                    throw std::runtime_error(
...
//                }
//
//                // 2) Генерируем значение rhs
//                llvm::Value *rhsVal = generateExpression(node.children[1]);
//
//                // 3) Делаем store
//                //    Если lhsPtr — глобальная переменная-скаляр, тогда dyn_cast<GlobalVariable>(lhsPtr) сработает,
//                //    но для массива GEP, это обычный pointer => CreateStore
//                if (auto *gvar = llvm::dyn_cast<llvm::GlobalVariable>(lhsPtr)) {
//                    builder.CreateStore(rhsVal, gvar);
//                } else {
//                    builder.CreateStore(rhsVal, lhsPtr);
//                }
//                return rhsVal; // иногда возвращают rhs, чтобы цепочку можно было использовать
//            }
//
...
//                llvm::Value *rhsVal = generateExpression(node.children[1]);
//
//                // Упрощённо поддержим только i32
//                if (!lhsVal->getType()->isIntegerTy(32) ||
//                    !rhsVal->getType()->isIntegerTy(32)) {
//                    throw std::runtime_error("BinaryOp: only i32 supported in this example (arith)");
//                }
...
//            }
//
//            // ---------------- 3.4) Логические операторы (&&, ||) (упрощённо) ----------------
//            if (op == "&&" || op == "||") {
//                llvm::Value *lhsVal = generateExpression(node.children[0]);
//                llvm::Value *rhsVal = generateExpression(node.children[1]);
//
//                // Приводим i32->i1 при необходимости
...
//                    }
//                    throw std::runtime_error(
//                            "Unsupported type for '&&'/'||' - need i1 or i32"
//                    );
//                };
//
...
//            }
//
//            // ---------------- 3.6) Оператор индексирования массива: [] ----------------
//            if (op == "[]") {
//                // чтение массива array[index] => нужно сделать GEP + load
//                // (если используется как rvalue).
//                //
//                // Если узел '[]' появляется слева в "=", то там (в ветке op=="=")
//                // мы вызываем generateArrayIndexPointer, возвращаем pointer и делаем Store.
//                //
//                // А здесь, в "rvalue" контексте, мы делаем:
//                llvm::Value *elemPtr = generateArrayIndexPointer(node);
//
//                // Загрузим i32
//                return builder.CreateLoad(builder.getInt32Ty(), elemPtr, "elemload");
//            }
//
//            // Если дошли сюда — оператор не поддержан
    ...
    switch (node.type) {
        ...
        case ASTNodeType::BinaryOp: {
            ...
            if (op == "[]") {
                ...
                // Нужно 3 аргумента: (Value*, bool isVolatile, Twine)
                // Надо знать elemTy, например i32
